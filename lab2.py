...
def nestsum(lst):  # задача 4 - замена всех вложенных списков суммой их элементов
    ...
    for item in lst:
        if isinstance(item, list):
            try:  # попытка просуммировать все элементы, если все норм то сумму добавляем в конечный массив
                elem = sum(item)
                flat_list.append(elem)
            except TypeError:  # ошибка, которая вылезает при наличии не-списков в подсписке
                sub_list = [flatten(item)]  # превращение "разнородного" списка в один подсписок
                flat_list.extend(nestsum(sub_list))  # этот подсписок снова отправляется на суммирование
        else:
...
def sequence(lst):  # задача 5 - наибольшая возрастающая подпоследовательность
    ...
    for i in range(len(lst) - 1):
        # проверка, является ли следующий элемент больше предыдущего на 1
        if lst[i + 1] == lst[i] + 1:
            ...
            temp.append(lst[i + 1])
            if save <= temp:  # если сохраненная последовательность меньше временной, то записываем в нее temp
                save = temp
        else:
            temp = []
    return save
...
def string_sum_map(text):  # задача 10 - сумма чисел в строке через map
    save = re.findall('\d+', text)  # поиск чисел в строке и запись их в список (/d+ - последовательность цифр)
    return sum(map(int, save))  # перевод элементов в числа через map и их суммирование
